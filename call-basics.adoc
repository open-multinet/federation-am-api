...
Note also that servers may respond with other HTTP error codes, and clients must be prepared to deal with those situations. 
Specifically, a server that is busy might return HTTP code 503 or 504, or just refuse the connection. 

*TODO* The layout below needs some work (a table isn't very nice either)

Below is a detailed description of each error.
...
Below is an example of a successful reply to a +Delete+ call.
[source]
------------------
{ 
   "output": "",
   ...
   [ 
       { 
          "geni_sliver_urn": "urn:publicid:IDN+example.com+sliver+5042",
          "geni_allocation_status": "geni_unallocated",
          "geni_error": "",
          "geni_expires": "2014-12-31T23:59:59Z"
       } 
   ] 
}
------------------

And below is exactly the same reply, but showing the characters received over the TCP connection. This shows HTTP and Xml-RPC details.
[source]
------------------
HTTP/1.1 200 OK
Date: Thu, 19 Jun 2014 08:20:27 GMT
...
<value><struct>
<member><name>geni_sliver_urn</name><value><string>urn:publicid:IDN+example.com+sliver+5042</string></value></member>
<member><name>geni_expires</name><value><string>2014-12-31T23:59:59Z</string></value></member>
<member><name>geni_error</name><value><string></string></value></member>
<member><name>geni_allocation_status</name><value><string>geni_unallocated</string></value></member>
</struct></value>
</data></array></value>
</member>
...
Below are 3 possible answers when the server is busy:
[source]
------------------
HTTP/1.1 504 Gateway Time-out
Date: Mon, 02 Jun 2014 14:11:39 GMT
Server: Apache/2.2.22 (FreeBSD)
Content-Length: 328
...
<hr>
<address>Apache/2.2.22 (FreeBSD) Server at example.com Port 443</address>
</body></html>
------------------
...
------------------
HTTP/1.1 200 OK
Date: Mon, 02 Jun 2014 14:10:50 GMT
Server: Apache/2.2.22 (FreeBSD)
Keep-Alive: timeout=15, max=100
Connection: Keep-Alive
Content-Type: text/xml
...
<fault>
<value><struct>
<member><name>faultString</name><value><string>Server is WAY too busy; please try again later</string></value></member>
...
------------------
{ 
   "output" -> "server is busy; try again later",
   "code" ->
   {
        "geni_code" -> -32001
   },
...
Additionally, the resource or slice itself might also be busy:
[source]
------------------
{ 
   "output" -> "resource is busy; try again later",
   "code" ->
   {
        "geni_code" -> 14,
        "am_type" -> "protogeni",
        "am_code" -> 14,
        "protogeni_error_url" -> "https://example.com/spewlogfile.php3?logfile=fc117247506e8b19b973865446eaa928",
        "protogeni_error_log" -> "urn:publicid:IDN+example.com+log+fc117247506e8b19b973865446eaa928"
   },
   "value" -> 0
}
------------------

In all of these "busy" cases, the client should try again after some time.


Finally, an example of an error reply to a +Status+ call.
[source]
------------------
{ 
...
Type name in this document:: +string.datetime.rfc3339+
***********************************

//////////////////////
refer to this using:
<<StringDateTimeRfc3339, +string.datetime.rfc3339+>>
//////////////////////
...
In the specification of this API, this type is refered to as +string.dateTime.rfc3339+.
...
Type name in this document:: +string.rspec+
***********************************

//////////////////////
refer to this using:
<<StringRspecDataType, +string.rspec+>>
//////////////////////
...
A fully compliant aggregate will always support the GENI standard schemas for RSpecs, see the link:rspec.html[Rspec section]. The current GENI RSpec version is 3 (type is geni, case insensitive). Aggregates are free to use an alternate format internally, but must accept and produce compliant RSpecs on demand.

This API requires this contract: Aggregates advertise the type and version of RSpec formats that they support in <<GetVersion>>. If available, they specify the schema, namespace and extensions combination which is the authoritative definition of that format. Clients of the API should understand that combination in order to know how to understand the resources available at that aggregate.
...
In this API, such RSpec fields are refered to as +string.rspec+. 
...
Several methods take some URNs to identify what to operate on. 
These methods are defined as accepting a list of arbitrary strings called URNs, 
which follow the GENI identifier rules. 
This API defines two kinds of URNs that may be supplied here, 
slice URNs and sliver URNs 
(see the GENI identifiers page). 
Some aggregates may understand other URNs, 
but these are not defined or required here. 
Aggregates that accept only URNs defined by this API will return an error when given URNs not in one of those forms. 
This API requires that aggregates accept either a single slice URN, or 1 or more sliver URNs that all belong to the same slice. 
Aggregates are not required to accept both a slice URN and sliver URNs, 2 or more slice URNs, or a set of sliver URNs that crosses multiple slices. 
Some aggregates may choose to accept other such combinations of URNs. 
Aggregates that accept only arguments defined by this API will return a BADARGS error when given more than 1 slice URN, 
a combination of both slice and sliver URNs, or a set of sliver URNs that belong to more than 1 slice.

If the +urns+ list includes a set of sliver URNs, then the AM shall apply the method to all listed slivers. 
If the operation fails on one or more of the slivers for any reason, 
then the whole method fails with an appropriate error code, 
unless +:best_effort+ is true and supported. 


If the list contains a sliver URN, and that sliver is deleted, expired or unknown, the call will result in a SEARCHFAILED error code. The +value+ (which is optional, since this is an error) may optionally contain more details, such as info on the expired sliver.

If the list contains a slice URN, and that slice has no slivers, or only expired or deleted slivers, the call results in a SEARCHFAILED error code. The +value+ (which is optional, since this is an error) may optionally contain more details, such as info on the expired or deleted slivers.

Note that for both cases above, it is possible that the AM has no info on expired or deleted slivers. because of this, the SEARCHFAILED error is the best error that can be guaranteed to be returned by calls that take a +urns+ argument. Some aggregate might have info on expired slivers. They should NOT return an EXPIRED error in the cases above. Aggregates should also not return a more general error.
Aggregates should also never return SUCCESS in the cases above, and just omit the sliver from their +value+.

//////////////////////////////////
In the scetions about the specific calls, refer to the above using: 
...
//////////////////////////////////
Overview of what could be returned when for AMv3:
|==========================
| arg | sliver status | return Describe | return Status & Renew & Delete | return Provision | return POA
| sliver urn | expired | SEARCHFAILED, EXPIRED or ERROR | same | same | same 
| sliver urn | deleted | SEARCHFAILED, EXPIRED or ERROR | same | same | same 
| sliver urn | neverexisted | SEARCHFAILED, EXPIRED or ERROR | same | same | same 
| slice urn | expired | empty :rspec and :sliver empty or containing expired | SUCCESS and :sliver empty or containing expired, OR EARCHFAILED or EXPIRED | SEARCHFAILED | SUCCESS and :sliver empty OR EARCHFAILED 
| slice urn | deleted | empty :rspec and :slivers empty or containing deleted | SUCCESS and :sliver empty or containing deleted, OR EARCHFAILED or EXPIRED | SEARCHFAILED | SUCCESS and :sliver empty OR EARCHFAILED 
| slice urn | neverexisted | empty :rspec and :slivers empty | SUCCESS and :sliver empty, OR EARCHFAILED or EXPIRED | SEARCHFAILED | SUCCESS and :sliver empty OR EARCHFAILED 
|==========================
//////////////////////////////////
...
XML-RPC type:: +string+
String content type::  <<StringDateTimeRfc3339, +string.datetime.rfc3339+>>
***********************************
...
The +:end_time+ argument/option requests an expiration of the specified slivers. It is in <<StringDateTimeRfc3339, +string.datetime.rfc3339+>> format.
...
***********************************
[horizontal]
Calls:: <<PerformOperationalAction>>, <<Renew>>, <<Delete>>
Calls:: (as part of larger +struct+) <<Allocate>>, <<Provision>>, <<Status>>, <<Describe>>
Supported by server:: Mandatory (part of the mandatory return of these calls)
XML-RPC type::
[source]
  [
    {
...
The +:operational_status+ in the sliver info list is required, unless these 2 conditions are met: 
...
==== Extended return +struct+

***********************************
[horizontal]
Calls:: <<Allocate>>, <<Provision>>, <<Describe>>, <<Status>>
Supported by server:: Mandatory (part of the mandatory return of these calls)
XML-RPC type::
[source]
    {
      ":rspec" : <string.rspec: manifest rspec>
      ":urn" : <string: slice urn of the containing slice>
      ":slivers" : <array: sliver info list described above>
    }
***********************************

Additional rules:

* +:urn+ only needs to be included for the +Describe+ and +Status+ calls.
* +:rspec+ only needs to be included for the +Allocate+, +Provision+ and +Describe+ calls.

==== Examples

.Example Call & Reply without +:best_effort+
[source]
------------------
  call:
  Renew(["urn:publicid:IDN+example.com+sliver+1", "urn:publicid:IDN+example.com+sliver+2"], 
         [ { <valid credential struct> } ], 
  ...
  reply:
  {
      "output" : "sliver urn:publicid:IDN+example.com+sliver+2 does not exist",
      "code" : {
                ":code" : 12
              },
      "value" : { }
...
.Example Call & Reply without +:best_effort+ (optionally including value)
[source]
------------------
  call:
  Renew(["urn:publicid:IDN+example.com+sliver+1", "urn:publicid:IDN+example.com+sliver+2"], 
  ...
  reply:
  {
      ...
      "value" : {  
            ":slivers":
               [
                 {
                  ":sliver_urn": "urn:publicid:IDN+example.com+sliver+1",
                  ":allocation_status": ":provisioned",
                  ":operational_status": ":notready",
                  ":expires": "2014-05-01T11:11:11Z",
                  ":error_code": 0 
                 },
                 {
                  ":sliver_urn": "urn:publicid:IDN+example.com+sliver+2",
                  ":allocation_status": ":unallocated",
                  ":error_code": 12,
                  ":error_output" : "no such sliver found here"
                 }
               ]
      }
  }
