...
NOTE: *TODO: Add some examples. These can be the same as examples later on in the document (but the corresponding example below will only include the date in the +value+ field). Also add an example in full "raw" xml, matching one of the other examples.*
