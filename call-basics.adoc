  ...
  [
    {
     ...
     (optional) ":error_code": <int: error code matching the failure or success of this sliver. The same code as in the default return structures code field is expected here>
     (optional) ":error_output": <string: explaining any failure for this sliver. The field may be omitted entirely but may not be null/None>
...
.Example Call & Reply with +:best_effort+ (aggregate cannot differentiate between expired, deleted and never existed)
[source]
------------------
  call:
...
.Example Call & Reply with +:best_effort+ (on aggregate that remembers expired and deleted slivers)
[source]
------------------
  call:
  Renew(["urn:publicid:IDN+example.com+sliver+1", "urn:publicid:IDN+example.com+sliver+2"], 
            ...
            ":slivers":
               [
                 ...
                 },
                 {
                  ...
                  ":allocation_status": ":unallocated",
                  ":error_code": 15,
                  ":error_output" : "sliver has expired"
                 }
               ]
      }
  }
