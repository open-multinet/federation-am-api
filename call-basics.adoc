...
+:allocate+:: A case insensitive string, one of fixed set of possible values. Default is +:single.+ This option defines whether this AM allows adding slivers to slices at an AM (i.e. calling Allocate() multiple times, without first deleting the allocated slivers). Possible values:
...
+:best_effort+:: <XML-RPC boolean 1/0, default 0>
...
+:api_versions+: an XML-RPC struct containing 1 or more entries of:
...
This section explains how specific data types are sent using XmlRpc.
The following primitive types are always sent using the matching XmlRpc data type:
- +string+
- +boolean+
- +int+
- +double+

Note that the following types are NOT used: +base64+ and +dateTime.iso8601+.
The +nil+ type is also not used (this is an xmlrpc extension).
...
example (in actual XML, not JSON)*.
...
NOTE: dates are NOT sent using the XmlRpc date type!

NOTE: _Wim Van de Meerssche:_  Dates are encoded with RFC3339 and send as XmlRpc +string+ type.
However, XmlRpc has a +dateTime.iso8601+ type. Why? Has this to do with library support?
...
If an Aggregate advertises a particular type/version (optionally defined with a combination of schema, namespace and extensions) in the +:request_rspec_versions+ attribute of GetVersion then it promises to correctly honor an Allocate (was CreateSliver in API v2) call containing a request RSpec in the given format, and then to return a Manifest RSpec in the corresponding format (i.e. a GENI format request is answered with a GENI format manifest). The aggregate also promises to send a correctly formatted Manifest RSpec in response to a Describe or Provision call which supplies a valid slice URN or list of sliver URNs and an +:rspec_version+ option containing that supported type/version.
...
*TODO this section was copied from the AMv3 spec with only minor changes (mostly markup). This sections content might still be moved to better places.*
*TODO: Does +Status+ support this option?+
*TODO: refer to <<OperationsOnIndividualSlivers, Operations on Individual Slivers>>*
...
[[CommonOptionEndTime]]
=== Common Option: +:end_time+

***********************************
[horizontal]
Name:: +:end_time+
Calls:: <<Allocate>>, <<Provision>>
Supported by the server:: Mandatory
Included by client:: Optional 
XmlRpc type:: +string+
String content type::  RFC 3339 date
Related:: +expiration_time+ argument of +Renew+ call, whichis an argument, and is mandatory to include by clients.
***********************************

*TODO this section was copied from the AMv3 spec with only minor changes (mostly markup). This sections content might still be moved to better places.*

The +:end_time+ argument requests an expiration of the specified slivers. It is in dateTime.!rfc3339 format (defined above). When an explicit argument, it is required, and aggregates must honor the request to the extent local policy permits. When an option in the options struct, clients may omit the option, and AMs may choose not to or be unable to honor this option, but may still succeed the overall request.
...
*TODO: this is a very common return. It should be described in general instead of at each call.
  ...
  [
    {
     ...
     ":resource_status" : optional string with resource-specific status in more detail than operational_status; may be omitted],
    },
    ...
  ],
------------------
