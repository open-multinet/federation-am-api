...
The name of the aggregate software or client tool that generated the request.
...
In manifest:: required (only 1 allowed)
type:: +xs:string+
***********************************
...
In manifest:: 0, 1 or more per node (same as request)
***********************************
...
The +client_id+ attribute is used to identify the interface.
Each +client_id+ must be globally unique: no other interface in the same RSpec may have the same +client_id+.

The +interface+ element can contain elements with more information. Such as for example IP address information:
[source]
------------------
...
+component_manager+ is an element for links, and not an attribute, 
as link between different sites have multiple component managers. 

This contains the URN of the component manager that manages the link.
